...
public class AdmissionCommiteeServiceTest {
	 ...
	 * @Test
	 * 
	 * @Order(2) public void testViewCommiteeMember() throws
	 * AdmissionMemNotFoundException{ when(repo.findById(id).get()).thenReturn(c1);
	 ...
	 * @Test
	 * 
	 * @Order(5) public void testUpdateCommiteeMember() throws
	 * AdmissionMemNotFoundException{ when(repo.findById(id).get()).thenReturn(c1);
	 * when(repo.findById(id)).thenReturn(Optional.of(c1)); AdmissionCommiteeMember
	 * cc1=repo.findById(id).get(); cc1.setAdminName("Tejas");
